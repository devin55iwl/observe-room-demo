    ...
    <div>
      ...
      >
        ...
        <img ref={bgImgRef} src={pipSwapped ? imgCookiyAI : imgInterviewee} alt=""
          ...
          style={{ filter: "brightness(0.88) contrast(1.04)", opacity: 0, pointerEvents: "none" }} />
        {/* Interviewee feed */}
        <img src={imgInterviewee} alt="" className="absolute inset-0 w-full h-full object-cover"
          style={{ filter: "brightness(0.88) contrast(1.04)", opacity: pipSwapped ? 0 : 1, transitionProperty: "opacity", transitionDuration: "0.6s", transitionTimingFunction: "ease" }} />
        {/* AI Moderator feed */}
        <img src={imgCookiyAI} alt="" className="absolute inset-0 w-full h-full object-cover"
          style={{ filter: "brightness(0.88) contrast(1.04)", opacity: pipSwapped ? 1 : 0, transitionProperty: "opacity", transitionDuration: "0.6s", transitionTimingFunction: "ease" }} />
        {/* Radial vignette */}
        <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse 80% 70% at 42% 42%, transparent 25%, rgba(10,12,18,0.55) 100%)" }} />
        {/* Vertical gradient */}
        <div className="absolute inset-0" style={{ background: "linear-gradient(180deg, rgba(10,11,15,0.25) 0%, transparent 30%, transparent 50%, rgba(10,11,15,0.60) 100%)" }} />
      </div>
